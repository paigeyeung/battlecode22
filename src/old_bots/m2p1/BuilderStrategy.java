...
strictfp class BuilderStrategy {
    ...
    static void runBuilder(RobotController rc) throws GameActionException {
        ...
        for (int i = 0; i < actionableAllies.length; i++) {
            ...
            if (allyRobot.getType().canAttack())
                attackUnitsCount ++;
        }
        ...
        if (attackUnitsCount == 0) {
            Direction dir = GeneralManager.getNextDir(rc,nearestAllyArchonLocation);
            if (dir != null && rc.canMove(dir)) {
                rc.move(dir);
            }
        }
        else if (actionableAllies.length > 5 || rc.getLocation().distanceSquaredTo(nearestAllyArchonLocation) < 3) {
            Direction dir = GeneralManager.getNextDir(rc,ArchonTrackerManager.getNearestEnemyArchon(rc.getLocation()).guessLocation);
            if (dir != null && rc.canMove(dir)) {
                rc.move(dir);
            }
        }
