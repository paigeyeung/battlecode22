...
//        rc.setIndicatorString("Hello world!");
    ...
    /**
     * Locally decides what a hostile droid should do
     */
...
//        }
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        HOSTILE_DROID_ACTIONS action = getHostileDroidAction(rc);
        if (action == HOSTILE_DROID_ACTIONS.ATTACK) {
            if (tryAttack(rc)) {
                // Try to attack
            }
            else {
                MapLocation visibleAttackTarget = getAttackTarget(rc, rc.getType().visionRadiusSquared);
                if (visibleAttackTarget != null) {
                    // Move towards nearest visible enemy
                    tryMove(rc, myLocation.directionTo(visibleAttackTarget), true);
                }
                else {
                    // If no enemies are visible, move towards nearest enemy Archon
                    tryMove(rc, myLocation.directionTo(getNearestEnemyArchon(myLocation).guessLocation), true);
                }
            }
        }
